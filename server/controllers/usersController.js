...
class UsersControllers {
    ...
    connection.query(sqlEmail, [email], (errEmail, result) => {
      if (errEmail) {
        ...
        if (result.length) {
          ...
          res.status(500).json("Email repetido");
        } else {
  ...
  verifyEmail = (req, res) => {
    ...
    if (!emailq) {
      res.status(406).json("Debes escribir un correo");
    }
  ...
  editUser = (req, res) => {
    ...
    connection.query(sql, values, (errSql, result) => {
      if (errSql) {
        ...
        return res
          .status(200)
          .json({ message: "Datos Modificados", user: name });
      }
    });
  };
  ...
  formEditPassword = (req, res) => {
    ...
    let sql =
      "SELECT password FROM user WHERE user_id = ? AND enabled_status = 2";
    connection.query(sql, [user_id], (err, result) => {
      if (err) {
  ...
  getOneUser = (req, res) => {
    ...
    connection.query(sql, [user_id], (err, result) => {
      ...
      if (!result.length) {
        return res
          .status(404)
          .json({ message: "Usuario no encontrado o deshabilitado" });
      }
